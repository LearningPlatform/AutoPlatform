...
from ..datahandle import rundata
...
def set_scheduler(scheduler,scheduler_ob):
    ...
    if plan_type == 1:
        scheduler.add_job(rundata.get_run_info, args=[run_data], trigger='interval', seconds=plan_interval,
                          id="plan_" + str(plan_id), start_date=start_time, end_date=end_time)
    else:
        scheduler.add_job(rundata.get_run_info, args=[run_data], trigger='date', run_date=start_time,
